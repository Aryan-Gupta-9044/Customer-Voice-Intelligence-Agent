...
st.set_page_config(page_title="Customer Voice Intelligence Agent", page_icon="­ƒôè", layout="wide")
...
def load_dataset(uploaded_file: Optional[Any] = None) -> pd.DataFrame:
...
def analyze_and_store(uploaded_file: Optional[Any], source_label: str) -> None:
    ...
    with st.spinner("Running cleaning, sentiment, embedding, theme and trend agents..."):
        ...
        st.session_state["analysis_results"] = run_analysis(dataframe)
...
def render_hero(source_label: str, row_count: int) -> None:
    st.markdown(
        f"""
        <div class="cva-hero">
            <h1>­ƒôè Customer Voice Intelligence Agent</h1>
...
def render_metrics(sentiment_frame: pd.DataFrame) -> None:
    ...
    cols = st.columns(5)
    cards = [
        ("­ƒôØ Total Reviews", f"{len(sentiment_frame):,}"),
        ("­ƒÖé Positive", int(counts.get("Positive", 0))),
        ("­ƒÿÉ Neutral", int(counts.get("Neutral", 0))),
        ("Ôÿ╣´©Å Negative", int(counts.get("Negative", 0))),
        ("Ô¡É Avg Rating", f"{avg_rating:.2f}" if pd.notna(avg_rating) else "N/A"),
    ]
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.caption("Expected columns: Review, Product, Rating, Date")
        analyze_button = st.button("Analyze Reviews", type="primary", use_container_width=True)
        st.divider()
        if st.session_state.get("analysis_results"):
            st.success(f"Loaded {st.session_state.get('row_count', 0)} reviews from {st.session_state.get('data_source', 'sample data')}.")
    ...
    if analyze_button:
        ...
        analyze_and_store(uploaded_file, source_label)
    ...
    overview_tab, themes_tab, trends_tab, qa_tab, evidence_tab = st.tabs(
        ["­ƒôê Overview", "­ƒº® Themes", "­ƒôà Trends", "­ƒÆ¼ Ask a Question", "­ƒöì Evidence Explorer"]
    )
